...
func (vm *VM) executeBinaryOperation(op code.Opcode) error {
	...
	case leftType == object.IntegerObj && rightType == object.IntegerObj:
		return vm.executeBinarIntegerOperation(op, left, right)
	...
	case leftType == object.StringObj && rightType == object.IntegerObj:
		return vm.executeStringRepeater(op, left, right)
	case leftType == object.IntegerObj && rightType == object.StringObj:
		return vm.executeStringRepeater(op, right, left)
	default:
		return fmt.Errorf("unsupported types for binary operation: %s %s", leftType, rightType)
	}
}

func (vm *VM) executeStringRepeater(op code.Opcode, left, right object.Object) error {
	if op != code.OpMul {
		return fmt.Errorf("unknown string operator: %d", op)
	}
	leftValue := left.(*object.String).Value
	rightValue := right.(*object.Integer).Value
	res := strings.Repeat(leftValue, int(rightValue))
...
func (vm *VM) executeBinarIntegerOperation(op code.Opcode, left, right object.Object) error {
